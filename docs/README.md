...
2. > python result_process.py
   1. After receiving classification HIT results, worker_quality(df) reads those results as a df to generate worker quality based on gold standard label performance and also computes a confusion matrix for each worker It generates data/analysis/gold_standard_quality.csv which includes columns for number of tasks completed, total accuracy, and whether or not the worker is good (90 percent accuracy for all categories)
...
4. > cd webapp && python app.py
   ...
   2. Loads our saved model (The model must be downloaded from our S3 bucket though the url specified in src/models/models.txt and placed in the webapp directory)
